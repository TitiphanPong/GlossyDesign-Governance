...
3. Inspect Native ChatGPT Scheduled Task state and classify continuation health. Current implementation coverage is the single persistent recurring implementation runner; historical one-time runway/recovery tasks are incident history only.
4. If safe actionable work remains, require the recurring implementation runner to be enabled and future-scheduled before relying on automation coverage. A one-time bootstrap wake may start work sooner, but it is not a successor chain and never counts as the persistent coverage mechanism.
5. Resume a safe actionable `IN_PROGRESS` phase first; otherwise select the next safe approved `OPEN` phase/TODO.
6. Acquire/resume the relevant durable goal with lnwjud scheduled continuation disabled because the project-level recurring runner owns continuation transport. If another healthy worker owns it, do not compete or mutate; return naturally and let the recurring runner retry on its next firing.
...
- Continuation no longer depends on pre-arming or replenishing one-time successors. If safe actionable work remains at the true turn boundary, checkpoint the exact phase/progress/next action/evidence, release the durable lease, and leave the same recurring implementation runner enabled for its next firing.
- `CHAIN_BROKEN` now means actionable work exists but the persistent recurring implementation runner is missing, disabled, or host state cannot verify future recurrence. Historical one-time runway/recovery tickets must not be recreated as the repair strategy.
- When all safe actionable work becomes terminal during a wake, or only `BLOCKED`/`REVIEW`/Needs Decision/approval-gated work remains, the recurring runner performs a no-op; disable it only after host verification when the owner intentionally wants implementation automation stopped.
- Never use Windows Task Scheduler, cron, shell timers, DOM automation, or a second local execution queue. Exactly one Native ChatGPT recurring implementation runner is the normal project-level continuation transport.
