...
- Glossy implementation automation uses a Native ChatGPT **one-time rolling continuation runway**, not an hourly recurring implementation worker. Each implementation wake should perform useful work continuously, targeting roughly 20–25 minutes when host/tool budget permits; the separate hourly recovery watchdog is orchestration-only and cannot implement project work.
...
- **Continuation-runway invariant:** while safe actionable `IN_PROGRESS`/`OPEN` implementation work remains, maintain a redundant Native ChatGPT **one-time rolling runway** instead of a single successor. When seeding or recovering from zero coverage, create three future enabled one-time tickets at approximately **+5, +15, and +30 minutes**. During ordinary wakes, do not recreate those offsets from the current clock; keep the existing future tickets and replace only consumed/missing capacity with a new ticket **beyond the current furthest future ticket**, preserving three distinct future tickets spanning roughly the next 30–40 minutes. Also maintain one separate hourly Native ChatGPT **recovery watchdog** that is orchestration-only and must never mutate application source. Historical/disabled/Complete tasks, past-due tasks, and the currently firing one-time ticket do not count as future runway coverage.
- Every wake must inspect and **top up the runway before workspace mutation or long-running work**. Missing runway capacity is created and host-verified; already-valid future tickets are reused; accidental same-slot/near-duplicate tickets are reconciled without deleting the intended three-slot redundancy. A wake must not spawn a new near-term branch merely because one future ticket is not exactly +5/+15/+30 relative to that wake. Any implementation-capable wake must still acquire the relevant durable lnwjud lease with `scheduledContinuation=off` before mutation. If another healthy worker owns the lease, the wake may repair scheduling coverage but must not mutate project source/state. The recovery watchdog only inspects TODO/durable-goal/host-task liveness and repairs missing runway coverage; it is not a second implementation queue.
- If work remains unfinished at the true turn boundary, checkpoint exact phase/progress/nextAction/evidence and leave the redundant runway in place. If all safe actionable work becomes terminal during the wake, or only `BLOCKED`/`REVIEW`/Needs Decision/approval-gated work remains, make all future implementation-runway tickets and the recovery watchdog non-runnable and host-verify cleanup before treating the chain as stopped.
- A one-time Scheduled Task becoming `Complete` means that wake fired/completed only; it does not imply TODO/backlog completion. Continuation health is determined from the remaining future rolling runway plus the recovery watchdog, not from the lifecycle state of the consumed ticket.
