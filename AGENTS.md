...
3. Inspect Native ChatGPT Scheduled Task state for the Glossy implementation runway and classify it using the schedule-context diagnostics (`RUNWAY_HEALTHY`, `RUNWAY_DEGRADED`, `CHAIN_BROKEN`, `CHAIN_DUPLICATE`, or `CHAIN_TERMINAL`). The currently firing one-time task, historical/disabled/Complete tasks, and past-due tickets do not count as future runway coverage.
...
5. If safe actionable work remains, before any workspace mutation or long-running project work maintain three future enabled Native ChatGPT one-time rolling-runway tickets and host-verify them. Seed/recover an empty runway at approximately +5, +15, and +30 minutes; on ordinary wakes preserve valid future tickets and replace only consumed/missing capacity with a new ticket beyond the current furthest future ticket. Do not recreate all three offsets from every wake. Also ensure the separate hourly Native ChatGPT recovery watchdog is enabled; it is orchestration-only and must never implement application work.
6. Resume the highest-priority safe actionable `IN_PROGRESS` phase first; otherwise continue the next safe approved `OPEN` phase/TODO.
7. Before any workspace mutation, acquire/resume the relevant durable goal lease with lnwjud scheduled continuation disabled for this one-time-runway policy. Never create a competing worker for an already healthy lease owner. A wake may repair runway coverage without the implementation lease but must not mutate project source/state.
8. At a true turn boundary, unfinished actionable work keeps the verified runway. If all safe actionable work is terminal, or only blocked/review/decision/approval-gated work remains, make all future runway tickets and the recovery watchdog non-runnable and host-verify cleanup.
9. Repair missing continuation coverage only according to the approved Native ChatGPT scheduling rules. `docs/SCHEDULE_CONTINUATION_CONTEXT.md` remains diagnostic/operational context; the redundant-runway hardening promoted into `DECISIONS.md` / `PROJECT_RULES.md` is normative.
