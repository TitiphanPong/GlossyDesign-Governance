...
3. Inspect Native ChatGPT Scheduled Task state for the single persistent Glossy recurring implementation runner and classify continuation health. Historical one-time runway/recovery tickets are incident history only and do not count as current coverage.
4. Treat workspace-root `TODO.md` and durable goal state as work truth; a Scheduled Task firing/completing is never sufficient evidence that a phase/TODO is complete.
5. If safe actionable work remains, require the recurring implementation runner to be enabled and future-scheduled before relying on automated continuation. A one-time bootstrap wake may start work sooner but is not a successor chain and never replaces the recurring runner.
...
7. Before any workspace mutation, acquire/resume the relevant durable goal lease with `scheduledContinuation=off`, because the project-level recurring runner owns continuation transport. Never create a competing worker for an already healthy lease owner.
8. At a true turn boundary, unfinished actionable work is checkpointed and the lease is released while the same recurring runner remains enabled. If only blocked/review/decision/approval-gated work remains, the runner performs a no-op unless the owner explicitly pauses/disables automation.
9. Repair missing continuation coverage by restoring/verifying the recurring runner, not by recreating historical one-time runways. `docs/SCHEDULE_CONTINUATION_CONTEXT.md` remains diagnostic/operational context; DEC-024 and active project rules are normative.
