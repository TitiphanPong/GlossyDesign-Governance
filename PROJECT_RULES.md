...
- The implementation TODO Runner uses exactly one persistent Native ChatGPT **recurring implementation runner** for this workspace. A recurring firing should perform useful work continuously while the host/tool budget permits; the host task itself remains scheduled after that firing completes.
- At the start of every wake, before workspace mutation or long-running project work, inspect concrete TODO/durable-goal/lease/worker/tracked-task/Native Scheduled Task liveness. While safe actionable work remains, continuation coverage is healthy only when the recurring implementation runner is enabled and future-scheduled. Historical one-time runway/recovery tasks do not count as current coverage.
- The recurring runner is the project-level continuation transport across phase/TODO boundaries. Individual implementation durable goals must be acquired with `scheduledContinuation=off` so a second per-goal watchdog is not created. If another healthy implementation worker owns the lease, do not compete or mutate; return naturally and let the same recurring runner fire again on its next cadence.
- An optional Native ChatGPT one-time bootstrap wake may be used only to restart work sooner than the next recurring firing. It is not a chain link, must not create a successor/runway, and does not replace the persistent recurring runner.
- If actionable work remains at the true turn boundary, checkpoint the exact current phase/progress/next action/evidence and release the durable lease. No successor creation is required because the recurring host task persists independently of the current wake.
- A recurring firing completing means only that occurrence completed; it does not mean the TODO/backlog completed and it must not disable the recurring runner.
- When no safe actionable `IN_PROGRESS`/`OPEN` work remains, or only `BLOCKED`/`REVIEW`/Needs Decision/approval-gated work remains, the recurring runner must perform a no-op. Disable it only after host verification when the owner intentionally wants implementation automation stopped.
