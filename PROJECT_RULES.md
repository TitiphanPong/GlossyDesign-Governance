...
- Before a wake mutates workspace state, it must inspect concrete goal/lease/worker/tracked-task/continuation liveness and must not create a competing worker when another healthy implementation worker is genuinely active.
- If actionable TODO work remains when the wake must end, checkpoint the exact current phase/progress/next action/evidence and create exactly **one** fresh Native ChatGPT **one-time** successor for approximately **5 minutes later** before returning. Never leave unfinished actionable work with neither a live blocking worker nor a confirmed successor.
- A fired one-time Scheduled Task becoming `Complete` means only that wake completed; it does not mean the TODO/backlog completed when a successor was required.
- Never allow more than one live successor for the same implementation chain. Do not create a new successor when a healthy worker or already-confirmed continuation owns the work.
